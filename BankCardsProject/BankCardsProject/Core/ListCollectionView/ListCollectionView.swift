...
protocol ListCollectionViewDelegate: AnyObject {
    @objc optional func listCollectionView(_ listCollectionView: ListCollectionView, didSelect item: AnyHashable, at indexPath: IndexPath)
}
...
class ListCollectionView: UICollectionView {
    ...
    private(set) lazy var diffableDataSource: UICollectionViewDiffableDataSource<ListSection, AnyHashable> = {
        let dataSource = UICollectionViewDiffableDataSource<ListSection, AnyHashable> (collectionView: self) { [weak self] collectionView, indexPath, itemIdentifier in
            self?
                .diffableDataSource
                .snapshot()
...
extension ListCollectionView: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        listDelegate?.listCollectionView?(self,
                                          didSelect:
                                            diffableDataSource.snapshot().sectionIdentifiers[indexPath.section].items[indexPath.item],
                                          at: indexPath)
    }
}
